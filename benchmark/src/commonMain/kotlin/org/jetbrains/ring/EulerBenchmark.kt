...
internal fun fibonacci(): Sequence<Int> {
    var a = 0
    var b = 1
    fun next(): Int {
        val res = a + b
...
internal fun Any.isPalindrome() = toString() == toString().reversed()

internal inline fun IntRange.sum(predicate: (Int) -> Boolean): Int {
    var sum = 0
    for (i in this) if (predicate(i)) sum += i
    return sum
}

internal inline fun Sequence<Int>.sum(predicate: (Int) -> Boolean): Int {
    var sum = 0
    for (i in this) if (predicate(i)) sum += i
    return sum
}
 ...
 */
internal open class EulerBenchmark {
