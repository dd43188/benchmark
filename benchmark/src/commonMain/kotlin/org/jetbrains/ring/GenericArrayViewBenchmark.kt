...
internal interface MemoryView<T> where T : Number {
    fun get(index: Int): T
}

internal class MemoryViewIntArray(val data: IntArray) : MemoryView<Int> {
    override fun get(index: Int): Int = data[index]
}

internal class MemoryViewLongArray(val data: LongArray) : MemoryView<Long> {
    override fun get(index: Int): Long = data[index]
}

internal class MemoryViewDoubleArray(val data: DoubleArray) : MemoryView<Double> {
    override fun get(index: Int): Double = data[index]
}

internal class Array2D<T>(val data: MemoryView<T>, val width: Int) where T : Number{
...
internal open class GenericArrayViewBenchmark {
    private val N = 2000

    private val intArr = Array2D(MemoryViewIntArray(IntArray(N * N) { Random.nextInt(119) }), N)
    // To confuse devirtualizer:
    private val longArr = Array2D(MemoryViewLongArray(LongArray(N * N) { Random.nextInt(119).toLong() }), N)
    private val doubleArr = Array2D(MemoryViewDoubleArray(DoubleArray(N * N) { Random.nextDouble(119.0) }), N)
