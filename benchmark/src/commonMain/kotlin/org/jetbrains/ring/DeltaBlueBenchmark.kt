...
internal fun alert(msg: String) {
  throw Error(msg)
}

/* --- O b j e c t   M o d e l --- */

internal class OrderedCollection<T> {
 ...
 */
internal abstract class Constraint(val strength: Strength) {
 ...
 * variable.
 */
internal abstract class UnaryConstraint(val myOutput: Variable, strength: Strength) : Constraint(strength) {
  var satisfied = false
 ...
 * optimization".
 */
internal class StayConstraint(v: Variable, str: Strength) : UnaryConstraint(v, str) {
 ...
 * wishes to change.
 */
internal class EditConstraint(v: Variable, str: Strength) : UnaryConstraint(v, str) {
  /**
 ...
 * variables.
 */
internal abstract class BinaryConstraint(val v1: Variable, val v2: Variable, strength: Strength) : Constraint(strength) {
 ...
 * read-only.
 */
internal class ScaleConstraint(src: Variable, val scale: Variable, val offset: Variable, dest: Variable, strength: Strength): BinaryConstraint(src, dest, strength) {
  /**
   * Adds this constraint to the constraint graph.
   */
  override fun addToGraph() {
 ...
 */
internal class EqualityConstraint(var1: Variable, var2: Variable, strength: Strength): BinaryConstraint(var1, var2, strength) {
  /**
   * Enforce this constraint. Assume that it is satisfied.
   */
  override fun execute() {
 ...
 * constraint solver.
 **/
internal class Variable(val name: String, var value : Int = 0) {
...
internal class DeltaBlueBenchmark {
  fun deltaBlue() {
    chainTest(100)
    projectionTest(100)
  }
