...
internal val MeanVariance.description: String
    get() {
        val format = { number: Double -> number.format(2) }
        return "${format(mean)} ± ${format(variance)}"
    }

internal val MeanVarianceBenchmark.description: String
    get() = "${score.format()} ± ${variance.format()}"

// Calculate difference in percentage compare to another.
internal fun MeanVarianceBenchmark.calcPercentageDiff(other: MeanVarianceBenchmark): MeanVariance {
...
internal fun MeanVarianceBenchmark.calcRatio(other: MeanVarianceBenchmark): MeanVariance {
...
internal fun geometricMean(values: Collection<Double>, totalNumber: Int = values.size) =
...
internal fun computeMeanVariance(samples: List<Double>): MeanVariance {
...
internal fun collectMeanResults(benchmarks: Map<String, List<BenchmarkResult>>): BenchmarksTable {
...
internal fun collectBenchmarksDurations(benchmarks: Map<String, List<BenchmarkResult>>): Map<String, Double> =
