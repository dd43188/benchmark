...
internal interface EntityFromJsonFactory<T> : ConvertedFromJson {
    fun create(data: JsonElement): T
}

// Parse array with benchmarks to list
internal fun parseBenchmarksArray(data: JsonElement): List<BenchmarkResult> {
...
internal open class BenchmarksReport(val env: Environment, benchmarksList: List<BenchmarkResult>, val compiler: Compiler) :
        JsonSerializable {
...
internal data class Compiler(val backend: Backend, val kotlinVersion: String) : JsonSerializable {
...
internal data class Environment(val machine: Machine, val jdk: JDKInstance) : JsonSerializable {
...
internal open class BenchmarkResult(val name: String, val status: Status,
...
internal data class MeanVariance(val mean: Double, val variance: Double)

// Processed benchmark result with calculated mean and variance value.
internal open class MeanVarianceBenchmark(name: String, status: BenchmarkResult.Status, score: Double, metric: BenchmarkResult.Metric,
...
internal open class BenchmarkWithStabilityState(name: String, status: BenchmarkResult.Status, score: Double, metric: BenchmarkResult.Metric,
